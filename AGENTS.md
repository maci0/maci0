...
2. **No model or AI tool names** in git-visible text (commit messages, branch names, PR titles and bodies, review comments, code comments): Claude, GPT, Copilot, Gemini, ChatGPT, Anthropic, OpenAI, Cursor, Codeium, and the rest.
...
4. **PR descriptions are minimal:** what changed, how to test. No "Summary of Changes" header, no per-file breakdown, no cheerleading.
...
5. Run the full gate. Fix regressions at once; never stack work on a red tree.
...
- **Baseline and candidate must match** in workload, seed, duration, inputs, collectors, and config. A number from a different setup is not a comparison.
- **Never profile a debug build.** Optimization off can move a hot path by an order of magnitude and flip the profile.
- **A win in one metric is not acceptance.** Lower CPU with worse fidelity is a regression, and any regression past a few percent needs a reason, not a merge.
...
- Secrets come from the env or a gitignored local config, never from argv (world-readable in the process table), a commit, or a log line.
- Never commit credentials, session dumps, or key material. Third-party binaries and assets need their license; vendor assemblies do not belong in a repo at all.
- Untrusted input is anything crossing into the process: a reviewed repo's file names and contents, tool output, model output, user text. Sanitize before display; never splice it into a prompt, shell command, or SQL unfenced or unparameterized.
...
**Keep docs operational:** stable contracts, not diary entries. Broad rules in parent docs, concrete detail in child docs. Direct bullets with explicit names. Delete history instead of explaining it. Never duplicate a rule across files unless each scope needs its own version.
...
**Every project gates on `black`, `ruff`, and `mypy`**, run through `uv run` in pre-commit and CI. **Static types always:** annotations on every param and return, PEP 604 unions (`T | None`), specific generics (`dict[int, str]`, not bare `dict`), named aliases for complex types. Each remaining `Any` explains why narrowing is infeasible.
...
bunx oxlint                  # lint (see below); eslint may run beside it
```

The lockfile is `bun.lock`: no `package-lock.json` beside it, and drop `.nvmrc` and `engines.node` once a repo is on bun. `.env` loads on its own, so no `dotenv`. CI uses `oven-sh/setup-bun@v2` (and `astral-sh/setup-uv@v5` for uv), never `actions/setup-node`. `.npmrc`'s `ignore-scripts` has no bun equivalent and needs none: bun runs lifecycle scripts only for packages in `trustedDependencies`, so installs are inert by default.

A repo already standardized on another package manager keeps it; do not migrate unasked.

**Linting is `oxlint`** with [anti-slop](https://github.com/dmmulroy/anti-slop) and [@rikalabs/oxlint-standards](https://github.com/Rika-Labs/oxlint-standards) at `strict`. ESLint can run beside it for rules oxlint has no port of; oxlint is the fast default, not a ban on the other. Silencing moves to watch for: disabling an anti-slop rule, an empty `SAFETY:` comment, type laundering (`as any`, `as unknown as T`, a widened signature). Review `oxlint --fix` rewrites before committing: its one-var, numeric-separator, and comment-case changes have broken bit-exact ports.
...
Render Mermaid `.mmd` files to SVG and PNG with [beautiful-mermaid](https://github.com/lukilabs/beautiful-mermaid), installed globally via bun, not added to a project `package.json`:
